"""Provide the bundled S3 standard-audiogram prescription."""

from tiresias_workstation.domain.dsp_contract import DspParameterId
from tiresias_workstation.domain.prescriptions import (
    PRESCRIPTION_FORMAT_NAME,
    PRESCRIPTION_FORMAT_VERSION,
    Prescription,
    PrescriptionParameter,
    PrescriptionSource,
)


S3_PRESCRIPTION = Prescription(
    profile_id="S3",
    display_name="S3 standard audiogram",
    description=(
        "CAMFIT prescription for the Bisgaard S3 standard audiogram "
        "(moderate/severe)."
    ),
    format_name=PRESCRIPTION_FORMAT_NAME,
    format_version=PRESCRIPTION_FORMAT_VERSION,
    parameters=(
        PrescriptionParameter(
            DspParameterId.BAND_1_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_2_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_3_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_4_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_5_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_6_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_7_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.BAND_8_COMPRESSOR_LUT,
            bytes(
                (
                    ,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.PHASE_COMP_GAIN_1,
            bytes(
                (
                    0x04, 0x38, 0xFF, 0xAA,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.PHASE_COMP_GAIN_2,
            bytes(
                (
                    0x04, 0x38, 0xFF, 0xAA,
                )
            ),
        ),
        PrescriptionParameter(
            DspParameterId.PHASE_COMP_GAIN_3,
            bytes(
                (
                    0x04, 0x38, 0xFF, 0xAA,
                )
            ),
        ),
    ),
    expected_sha256="7c73b2c02ee42c1730953d320abece6cec03bbb26407a7c147d5e244e34717d0",
    source=PrescriptionSource(
        repository="tiresias-eval",
        path=(
            "tiresias-eval-sigma/scripts/generated/S3/"
            "S3_apply_prescription.sss"
        ),
        revision="e1cb13dd9e1e9a941575f9f836eb56418d4ff812",
        sha256="79f221cf595e31f93a22249a6381b64de8cd6229450debf081e54a36f05878b7",
    ),
)
