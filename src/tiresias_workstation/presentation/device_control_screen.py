"""Present connected-board identity and cataloged DSP parameter control.
...
class DeviceControlScreen(QWidget):
    ...
    def _build_parameter_tab(self) -> QWidget:
        """Build the catalog table and ID-based read/write controls."""
        page = QWidget()
        ...
        self._parameter_table.setHorizontalHeaderLabels(
            ["ID", "Name", "Access", "Range", "Default", "Stored value"]
        )
        ...
        self._selected_parameter = QLabel("Select a catalog parameter")
        ...
        self._message = QLabel("Stored values are read from internal flash after connection.")
    ...
    def _session_loaded(self, payload: object) -> None:
        """Render one validated device session and refresh stored values."""
        if not isinstance(payload, DeviceSession):
            return
        self._session = payload
        self._stored_values.clear()
        self._definitions = {
            definition.parameter_id: definition for definition in payload.catalog
        }
        ...
        self._board_values["layout"].setText(f"0x{protocol.layout_id:08X}")
        ...
        self._board_values["behavior"].setText(
            "Flash persistence enabled; DSP application deferred"
            if deferred
            else "Flash persistence and live DSP application"
        )
        self._summary.setText(
            f"{info.model_number or 'Tiresias device'} · {len(payload.catalog)} "
            "cataloged parameters · READY"
        )
        self._populate_catalog(payload.catalog)
    ...
    def _selection_changed(self) -> None:
        """Configure the editor from the selected catalog definition."""
        definition = self._selected_definition()
        if definition is None:
            self._selected_parameter.setText("Select a catalog parameter")
        else:
            self._selected_parameter.setText(
                f"#{definition.parameter_id} · {definition.name}"
            )
            self._value_input.setRange(
                _raw_to_linear(definition.minimum),
                _raw_to_linear(definition.maximum),
            )
            self._value_input.setSingleStep(_raw_to_linear(definition.step))
            stored = self._stored_values.get(definition.parameter_id)
            self._value_input.setValue(
                _raw_to_linear(stored.value if stored is not None else definition.default)
            )
    ...
    def _write_selected(self) -> None:
        """Persist the selected value after local catalog validation."""
        definition = self._selected_definition()
        if definition is None:
            return
        raw_value = round(self._value_input.value() * _Q5_23_SCALE)
        if not definition.accepts(raw_value):
            self._show_message("Value does not align with the catalog step.", True)
            return
    ...
    def _refresh_all(self) -> None:
        """Queue one read per catalog entry without overlapping requests."""
        if self._operation_active or self._session is None:
            return
        self._pending_reads = [entry.parameter_id for entry in self._session.catalog]
        self._read_next()
    ...
    def _write_started(self, parameter_id: int, _value: int) -> None:
        """Render a persistent write without claiming completion."""
        self._operation_started(parameter_id)
        self._show_message(f"Persisting parameter {parameter_id}…")

    @Slot(object)
    def _parameter_received(self, payload: object) -> None:
        """Render a correlated stored value and continue queued refresh."""
    ...
    def _parameter_written(self, payload: object) -> None:
        """Render only firmware-confirmed flash persistence as success."""
        if not isinstance(payload, ParameterValue):
            return
        if self._session is None:
            return
        ...
        self._show_message(
            f"Persisted in internal flash · revision {payload.parameter_revision}. "
            "DSP application is deferred in this MVP."
        )
        self._operation_finished()
    ...
    def _populate_catalog(
        self, catalog: tuple[ParameterDefinition, ...]
    ) -> None:
        """Render catalog metadata while intentionally hiding DSP addresses."""
        self._parameter_table.setRowCount(len(catalog))
        for row, definition in enumerate(catalog):
            ...
            access = []
            if definition.access & ParameterAccess.READABLE:
                access.append("Read")
            if definition.access & ParameterAccess.WRITABLE:
                access.append("Write")
            if definition.access & ParameterAccess.PERSISTENT:
                access.append("Flash")
            values = (
                identifier,
                QTableWidgetItem(definition.name),
                QTableWidgetItem(" · ".join(access)),
                QTableWidgetItem(
                    f"{_raw_to_linear(definition.minimum):.4f}–"
                    f"{_raw_to_linear(definition.maximum):.4f}"
                ),
                QTableWidgetItem(f"{_raw_to_linear(definition.default):.6f}"),
    ...
    def _set_parameter_value(self, value: ParameterValue) -> None:
        """Update the stored-value cell and selected editor."""
        ...
        for row in range(self._parameter_table.rowCount()):
            ...
            if item is not None and item.data(Qt.ItemDataRole.UserRole) == value.parameter_id:
                self._parameter_table.item(row, 5).setText(
                    f"{_raw_to_linear(value.value):.6f}"
                )
                if self._parameter_table.currentRow() == row:
                    self._value_input.setValue(_raw_to_linear(value.value))
                break

    def _selected_definition(self) -> ParameterDefinition | None:
        """Return the selected catalog definition by stable ID."""
    ...
    def _update_actions(self) -> None:
        """Derive action availability from selection, access, and busy state."""
        definition = self._selected_definition()
        connected = self._session is not None
        idle = connected and not self._operation_active
        self._read_button.setEnabled(
            bool(idle and definition and definition.access & ParameterAccess.READABLE)
        )
        self._write_button.setEnabled(
            bool(idle and definition and definition.access & ParameterAccess.WRITABLE)
        )
...
def _raw_to_linear(value: int) -> float:
    """Convert one signed Q5.23 wire value to a UI linear value."""
    return value / _Q5_23_SCALE
